...
class UrbanSoundDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mel_spec = mel_spec.unsqueeze(0)  # [1, num_mels, time]
...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x)))  # [B, 16, M/2, T/2]
        x = self.pool(F.relu(self.conv2(x)))  # [B, 32, M/4, T/4]
        x = self.pool(F.relu(self.conv3(x)))  # [B, 64, M/8, T/8]
        x = x.view(x.size(0), -1)
